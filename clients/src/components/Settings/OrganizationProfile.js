...
const OrganizationProfile = () => {
    ...
    if (item) {
      if (type === 'facility') {
        ...
        setNewItem({
          ...
          code: item.code || '',
          type: item.type || '',
          parentId: item.regionId || item.zoneId || item.woredaId || ''
        });
      }
    } else {
    ...
    try {
      if (dialogType === 'facility') {
        const facilityData = {
          ...
          facility_type: newItem.type,
          woreda_name: woredas.find(w => w.id === newItem.parentId)?.name || '',
      ...
      // For now, we'll handle them locally as before
        ...
        if (selectedItem) {
          setRegions(prev => prev.map(r => r.id === selectedItem.id ? { ...r, ...newItem } : r));
        } else {
  ...
  const getWoredaName = (woredaId) => woredas.find(w => w.id === woredaId)?.name || 'N/A';
  ...
  const getJobTitleStats = () => {
    ...
    // Sort job titles - put N/A, Unassigned, and empty values at the bottom
                      ...
                      {regions.map((region) => (
                        <TableRow key={region.id}>
                          ...
                          <TableCell align="center">
                            <IconButton onClick={() => handleOpenDialog('region', region)} color="primary">
                              <Edit />
                            </IconButton>
                            <IconButton onClick={() => handleDeleteItem('region', region.id)} color="error">
                              <Delete />
                            </IconButton>
                          </TableCell>
                        </TableRow>
                    ...
                    <TableBody>
                      {zones.map((zone) => (
                        <TableRow key={zone.id}>
                          ...
                          <TableCell align="center">
                            <IconButton onClick={() => handleOpenDialog('zone', zone)} color="primary">
                              <Edit />
                            </IconButton>
                            <IconButton onClick={() => handleDeleteItem('zone', zone.id)} color="error">
                              <Delete />
                            </IconButton>
                          </TableCell>
                        </TableRow>
              ...
              <Box>
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
                  ...
                  <Button
                    variant="contained"
                    startIcon={<Add />}
                    onClick={() => handleOpenDialog('woreda')}
                    className="action-button"
                  >
                    Add Woreda
                  </Button>
                    ...
                    <TableBody>
                      {paginatedWoredas.map((woreda) => (
                        <TableRow key={woreda.id}>
                          ...
                          <TableCell align="center">
                            <IconButton onClick={() => handleOpenDialog('woreda', woreda)} color="primary">
                              <Edit />
                            </IconButton>
                            <IconButton onClick={() => handleDeleteItem('woreda', woreda.id)} color="error">
                              <Delete />
                            </IconButton>
                          </TableCell>
                        </TableRow>
                      ...
                      {paginatedFacilities.map((facility) => (
                        <TableRow key={facility.id}>
                          ...
                          <TableCell align="center">
                            ...
                            <IconButton onClick={() => handleDeleteItem('facility', facility.id)} color="error">
                              <Delete />
                            </IconButton>
                          </TableCell>
                        </TableRow>
              ...
              {dialogType === 'facility' && (
                <>
                  ...
                  <FormControl fullWidth>
                    ...
                    >
                      {woredas.map((woreda) => {
                        const zone = zones.find(z => z.id === woreda.zoneId);
                        const region = regions.find(r => r.id === zone?.regionId);
                        const displayName = `${woreda.name} (${zone?.name || 'N/A'}, ${region?.name || 'N/A'})`;
                        
                        return (
                          <MenuItem key={woreda.id} value={woreda.id}>
                            {displayName}
                          </MenuItem>
                        );
                      })}
